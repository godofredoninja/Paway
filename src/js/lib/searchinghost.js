...
export default class SearchinGhost {
  ...
  constructor (args) {
    this.config = {
      ...
      postsFormats: [],
      indexedFields: ['title'],
      template: post => `<a class="flex items-center noWrapWithEllipsis" href="${post.url}">${post.name === undefined ? post.title : post.name}</a>`,
   ...
   */
  buildUrl (options) {
    let url = `${this.config.url}/ghost/api/content/posts/?key=${this.config.key}`
